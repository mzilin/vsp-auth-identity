...
The **Identity Service** is a core component of the **Video Streaming Platform**, dedicated to managing user authentication and secure credential storage. Its key responsibilities include:

- **User Authentication**: Validates user credentials and ensures secure access to the platform.
- **Password Management**: Handles password storage, hashing, and verification, ensuring robust security practices.
- **Passcode Management**: Manages one-time passcodes or temporary codes used for multi-factor authentication or verification processes.
- **Token Management**: Issues, stores, and validates reset tokens to enable secure password recovery.

By centralising these responsibilities, the Identity Service ensures that authentication processes are secure, scalable and maintainable—laying the foundation for a secure user experience across the platform.
...
- **Spring Boot**
  ...
  - **Security-Crypto**: Provides robust encryption and decryption functionalities to secure sensitive data, ensuring data privacy and integrity.
